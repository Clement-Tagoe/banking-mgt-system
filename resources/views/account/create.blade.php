    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-2 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('account.store', $user) }}" class="p-8">
                    @csrf
            
                    <!-- Account Number -->
                    <div>
                        <x-input-label for="account_number" :value="__('Account Number')" />
                        <x-text-input id="account_number" class="block mt-1 w-full" type="text" name="account_number" :value="old('account_number')" required autofocus autocomplete="account_number" />
                    ...
                    <div class="mt-4">
                        ...
                        <select name="type" id="type" class="h-10 py-1 px-2 rounded-md border border-gray-300 hover:border-gray-400 outline-none text-gray-700 placeholder-gray-400 w-full block mt-1">
                            ...
                            <option value="current account" {{ old('type') == 'current account' ? 'selected' : '' }}>Current Account</option>
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <select name="currency" id="currency" class="h-10 py-1 px-2 rounded-md border border-gray-300 hover:border-gray-400 outline-none text-gray-700 placeholder-gray-400 w-full block mt-1">
                            ...
                            <option value="$" {{ old('type') == '$' ? 'selected' : '' }}>USD</option>
                            <option value="€" {{ old('type') == '€' ? 'selected' : '' }}>EUR</option>
                            <option value="£" {{ old('type') == '£' ? 'selected' : '' }}>GBP</option>
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="time" class="block mt-1 w-full" type="text" name="time" :value="old('time')" required autocomplete="time" />
